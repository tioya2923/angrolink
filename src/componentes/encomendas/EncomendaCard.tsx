...
export function EncomendaCard({ encomenda, vendedor, rotaDetalhe = '/dashboard/encomendas' }: { encomenda: EncomendaResumo; vendedor: boolean; rotaDetalhe?: string }) {
  ...
  return <article className="painel-dashboard-item flex flex-col gap-4 p-4 sm:flex-row sm:items-center">
    ...
    <div className="min-w-0 flex-1 space-y-1">
      <div className="flex flex-wrap items-center gap-2"><h2 className="font-titulo font-bold">{encomenda.codigo_publico}</h2><Badge variant="outline" className={classeEstadoEncomenda(encomenda.estado)}>{rotuloEstadoEncomenda(encomenda.estado)}</Badge>{encomenda.modalidade_recebimento === 'entrega' && <Badge variant="outline" className="border-blue-200 bg-blue-50 text-blue-800"><Truck className="mr-1 size-3" />Entrega</Badge>}</div>
    ...
    <div className="flex items-center justify-between gap-3 sm:flex-col sm:items-end"><strong className="font-corpo text-green-800">{formatarCentimosAoa(encomenda.total_centimos)}</strong><Link to={`${rotaDetalhe}/${encomenda.id}`} className="inline-flex items-center gap-1 rounded-lg border border-green-200 px-3 py-2 text-sm font-semibold text-green-800 hover:bg-green-50">{vendedor ? 'Gerir encomenda' : 'Ver detalhes'} <ChevronRight className="size-4" /></Link></div>
  </article>;
}
