...
import { CircleAlert, MapPin, Package, Phone, Store, Truck, UserRound } from 'lucide-react';
...
import { obterMediaEntregaParticipante } from '@/services/mediaEntregaParticipante';
...
function EntregaResumo({ entrega, contexto, encomendaId }: { entrega: EntregaParticipante | null; contexto: ContextoDetalheEncomenda; encomendaId: string }) {
  const estado = entrega?.estado ?? 'nao_atribuido';
  if (contexto === 'cliente') return <ProgressoEntregaComprador entrega={entrega} estado={estado} encomendaId={encomendaId} />;
  ...
  return <div className="painel-dashboard-form">
    ...
    {['aceite','chegou_origem','recolhida','chegou_destino','concluida'].includes(estado) && <IdentidadeEntregador entrega={entrega} encomendaId={encomendaId} />}
...
function ProgressoEntregaComprador({ entrega, estado, encomendaId }: { entrega: EntregaParticipante | null; estado: EntregaParticipante['estado']; encomendaId: string }) {
  const veiculo = entrega?.veiculo;
  ...
  return <div className="painel-dashboard-form">
    ...
    {identidadeDisponivel && <IdentidadeEntregador entrega={entrega} encomendaId={encomendaId} />}
  </div>;
}

function IdentidadeEntregador({ entrega, encomendaId }: { entrega: EntregaParticipante | null; encomendaId: string }) {
  const [fotoEntregador, setFotoEntregador] = useState<string | null>(null);
  const [fotoVeiculo, setFotoVeiculo] = useState<string | null>(null);
  useEffect(() => { let ativo = true; if (entrega?.foto_entregador_disponivel) void obterMediaEntregaParticipante(encomendaId, 'foto_entregador').then(url => ativo && setFotoEntregador(url)).catch(() => ativo && setFotoEntregador(null)); if (entrega?.foto_veiculo_disponivel) void obterMediaEntregaParticipante(encomendaId, 'foto_veiculo_entregador').then(url => ativo && setFotoVeiculo(url)).catch(() => ativo && setFotoVeiculo(null)); return () => { ativo = false; }; }, [encomendaId, entrega?.foto_entregador_disponivel, entrega?.foto_veiculo_disponivel]);
  const veiculo = entrega?.veiculo;
  return <div className="mt-3 flex gap-3 rounded-xl border border-green-100 bg-green-50/60 p-3 text-sm"><div className="flex size-12 shrink-0 items-center justify-center overflow-hidden rounded-full bg-primary/10">{fotoEntregador ? <img src={fotoEntregador} alt="Foto do entregador" className="size-full object-cover" /> : <UserRound className="size-5 text-primary" />}</div><div className="min-w-0 flex-1 space-y-1"><p className="font-semibold text-green-950">{entrega?.nome_entregador}</p><p className="text-muted-foreground">{[veiculo?.tipo_veiculo, veiculo?.marca, veiculo?.modelo].filter(Boolean).join(' · ')}</p><p className="text-muted-foreground">Matrícula: {veiculo?.matricula || 'Por confirmar'}</p></div>{fotoVeiculo && <img src={fotoVeiculo} alt="Foto do veículo do entregador" className="size-16 rounded-lg border object-cover" />}</div>;
}
