...
export default function ClienteEncomendaDetalhe({ rotaVoltar = '/dashboard/encomendas' }: { rotaVoltar?: string }) {
  ...
  return <div className="space-y-5">
    ...
    <MensagensOperacionaisEntrega encomendaId={encomenda.id} estadoEncomenda={encomenda.estado} abas={[{ id: 'comprador_vendedor', titulo: 'Vendedor' }, ...(encomenda.modalidade_recebimento === 'entrega' && encomenda.entrega_participante?.atribuicao_id && (['recolhida', 'chegou_destino', 'concluida'].includes(encomenda.entrega_participante.estado) || (encomenda.entrega_participante.estado === 'cancelada' && Boolean(encomenda.entrega_participante.recolhida_em))) ? [{ id: 'comprador_entregador' as const, titulo: 'Entregador', atribuicaoEntregaId: encomenda.entrega_participante.atribuicao_id, bloquearEnvio: ['concluida', 'cancelada'].includes(encomenda.entrega_participante.estado) }] : [])]} />
    ...
    {encomenda.estado === 'concluida' && <section className="rounded-2xl border-2 border-green-200 bg-green-50 p-5"><h2 className="font-titulo text-lg font-bold text-green-950">Encomenda concluída</h2><p className="mt-2 text-sm text-green-900">Receção confirmada em {encomenda.concluido_em ? new Intl.DateTimeFormat('pt-AO', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(encomenda.concluido_em)) : 'data por confirmar'}.</p>{!disputa && <button disabled={acao} onClick={() => setProblemaAberto(true)} className="mt-4 rounded-lg border border-amber-400 px-4 py-2 text-sm font-semibold text-amber-900">Reportar problema</button>}<button onClick={() => setFeedbackAberto(true)} className="mt-4 ml-2 rounded-lg border border-green-700 px-4 py-2 text-sm font-semibold text-green-900">Dar feedback</button></section>}
    <FeedbackPilotoConvite contexto="cliente" encomendaId={encomenda.id} atribuicaoEntregaId={encomenda.entrega_participante?.atribuicao_id} concluida={encomenda.estado === 'concluida'} bloqueado={dialog || problemaAberto || feedbackAberto} aoDarFeedback={() => setFeedbackAberto(true)} />
    <FeedbackPilotoDialog aberto={feedbackAberto} aoFechar={() => setFeedbackAberto(false)} contexto="cliente" encomendaId={encomenda.id} atribuicaoEntregaId={encomenda.entrega_participante?.atribuicao_id} />
