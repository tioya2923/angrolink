...
export default function ClienteEncomendas({ titulo = 'Minhas encomendas', descricao = 'Acompanha cada pedido até à conclusão.', rotaDetalhe = '/dashboard/encomendas' }: { titulo?: string; descricao?: string; rotaDetalhe?: string }) {
  ...
  if (erro) return <div className="painel-dashboard-form text-sm text-destructive"><p>Não foi possível carregar as encomendas.</p><button type="button" onClick={() => void carregar()} className="mt-3 font-semibold underline">Tentar novamente</button></div>;
