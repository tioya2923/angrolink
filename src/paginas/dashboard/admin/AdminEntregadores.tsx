  ...
  return <div className="space-y-6">
    ...
    {aCarregar ? <p className="py-10 text-center font-corpo text-sm text-muted-foreground">A carregar parceiros de entregas…</p> : <div className="space-y-3">
      ...
      {visiveis.map(parceiro => <article key={parceiro.id} className="painel-dashboard-item overflow-hidden p-0">
        <div className="p-4 sm:p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex min-w-0 gap-3"><Avatar parceiro={parceiro}/><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="font-titulo text-base font-bold">{parceiro.nome_completo}</h2><span className={`rounded-full border px-2 py-0.5 font-corpo text-[11px] font-semibold ${estiloEstado[parceiro.estado]}`}>{rotuloEstado[parceiro.estado]}</span></div><p className="mt-1 flex items-center gap-1 font-corpo text-xs text-muted-foreground"><Truck className="size-3.5"/>{parceiro.veiculos_entrega?.map((v: any) => `${v.tipo_veiculo} · ${v.marca} ${v.modelo}`).join(' | ') || 'Veículo por confirmar'}</p><p className="mt-1 flex items-center gap-1 font-corpo text-xs text-muted-foreground"><MapPin className="size-3.5"/>{parceiro.municipio}, {parceiro.provincia}</p></div></div><button onClick={() => setDetalhe(detalhe === parceiro.id ? null : parceiro.id)} className="inline-flex items-center gap-1 self-start rounded-lg border border-primary/30 px-3 py-1.5 font-corpo text-xs font-semibold text-primary hover:bg-primary/5"><Eye className="size-3.5"/>{detalhe === parceiro.id ? 'Ocultar detalhes' : 'Ver dados completos'}</button></div>
...
function Detalhes({ parceiro, abrirDocumento, analisarDocumento }: { parceiro: Parceiro; abrirDocumento: (path?: string) => void; analisarDocumento: (parceiroId: string, documento: any, estado: 'aprovado' | 'rejeitado') => void }) { const veiculos = parceiro.veiculos_entrega || []; const documentos = parceiro.documentos_parceiro_entrega || []; const zonas = parceiro.areas_cobertura_entrega || []; return <div className="border-t border-border bg-muted/25 p-4 sm:p-5"><div className="grid gap-5 lg:grid-cols-2"><section><h3 className="mb-3 font-titulo text-sm font-bold">Dados de contacto</h3><dl className="grid grid-cols-2 gap-x-4 gap-y-3"><Info rotulo="Telefone" valor={parceiro.telefone}/><Info rotulo="E-mail" valor={parceiro.email || 'Não indicado'}/><Info rotulo="Contacto de emergência" valor={parceiro.contacto_emergencia}/><Info rotulo="Registo" valor={parceiro.criado_em ? new Date(parceiro.criado_em).toLocaleString('pt-PT') : ''}/><Info rotulo="Zona base" valor={[parceiro.bairro, parceiro.municipio, parceiro.provincia].filter(Boolean).join(', ')}/></dl></section><section><h3 className="mb-3 font-titulo text-sm font-bold">Veículo e capacidade</h3>{veiculos.map((v: any) => <div key={v.id} className="rounded-xl border border-border bg-card p-3"><p className="font-corpo text-sm font-semibold capitalize">{v.tipo_veiculo} · {v.marca} {v.modelo}</p><p className="mt-1 font-corpo text-xs text-muted-foreground">Matrícula: {v.matricula} · {v.cor} · {v.capacidade_kg} kg{v.capacidade_volume_m3 ? ` · ${v.capacidade_volume_m3} m³` : ''}</p><p className="mt-1 font-corpo text-xs text-muted-foreground">Caixa: {v.possui_caixa_carga ? 'Sim' : 'Não'} · Paletes: {v.aceita_paletes ? 'Sim' : 'Não'} · Refrigerado: {v.possui_refrigeracao ? 'Sim' : 'Não'}</p></div>)}</section></div><section className="mt-5"><h3 className="mb-1 font-titulo text-sm font-bold">Documentos enviados</h3><p className="mb-3 font-corpo text-xs text-muted-foreground">Abra frente e verso antes de aprovar ou rejeitar cada documento.</p><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{documentos.map((d: any) => <div key={d.id} className="rounded-xl border border-border bg-card p-3"><p className="font-corpo text-xs font-semibold">{nomesDocumento[d.tipo_documento] || d.tipo_documento}</p><p className="mt-1 font-corpo text-[11px] text-muted-foreground capitalize">Estado: {d.estado}</p><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => abrirDocumento(d.frente_path)} className="rounded-md border border-primary/30 px-2 py-1 font-corpo text-xs text-primary hover:bg-primary/5">Ver frente</button><button onClick={() => abrirDocumento(d.verso_path)} className="rounded-md border border-primary/30 px-2 py-1 font-corpo text-xs text-primary hover:bg-primary/5">Ver verso</button></div><div className="mt-2 flex gap-2">{d.estado !== 'aprovado' && <button onClick={() => analisarDocumento(parceiro.id, d, 'aprovado')} className="rounded-md border border-primary px-2 py-1 font-corpo text-xs text-primary hover:bg-primary hover:text-primary-foreground">Aprovar</button>}<button onClick={() => analisarDocumento(parceiro.id, d, 'rejeitado')} className="rounded-md border border-destructive px-2 py-1 font-corpo text-xs text-destructive hover:bg-destructive hover:text-destructive-foreground">{d.estado === 'aprovado' ? 'Rejeitar após revisão' : 'Rejeitar'}</button></div>{d.motivo_rejeicao && <p className="mt-2 font-corpo text-xs text-destructive">{d.motivo_rejeicao}</p>}</div>)}</div>{documentos.length === 0 && <p className="font-corpo text-sm text-muted-foreground">Nenhum documento foi enviado.</p>}</section><section className="mt-5"><h3 className="mb-2 font-titulo text-sm font-bold">Áreas de cobertura</h3><div className="flex flex-wrap gap-2">{zonas.map((z: any) => <span key={z.id} className="rounded-full border border-primary/20 bg-primary/5 px-3 py-1 font-corpo text-xs text-primary"><MapPin className="mr-1 inline size-3"/>{[z.bairro, z.municipio, z.provincia].filter(Boolean).join(', ')}</span>)}</div></section>{parceiro.motivo_rejeicao && <p className="mt-4 rounded-lg border border-destructive/30 bg-destructive/5 p-3 font-corpo text-sm text-destructive"><strong>Motivo da rejeição:</strong> {parceiro.motivo_rejeicao}</p>}{parceiro.motivo_suspensao && <p className="mt-4 rounded-lg border border-orange-500/30 bg-orange-500/5 p-3 font-corpo text-sm text-orange-700"><strong>Motivo da suspensão:</strong> {parceiro.motivo_suspensao}</p>}</div>; }
