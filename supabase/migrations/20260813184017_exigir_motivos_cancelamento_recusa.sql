-- A migration original de encomendas já está aplicada. Esta revisão preserva
-- assinatura, RLS e transições, endurecendo apenas os motivos de encerramento.
begin;
...
begin
  ...
  values (
    ...
    case
      when v_evento in ('cliente_cancelou', 'vendedor_recusou')
        then jsonb_build_object('motivo', v_motivo)
      else '{}'::jsonb
    end
  );
